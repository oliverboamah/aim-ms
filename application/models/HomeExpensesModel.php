...
class HomeExpensesModel extends CI_Model
{
	public function store($data)
	{
		return $this->db->insert('home_expenses', $data);
	}

	public function update($expense_id, $data)
	{
		$this->db->where('expense_id', $expense_id);
		return $this->db->update('home_expenses', $data);
	}

	public function find_row($data)
	{
		return $this->db->get_where('home_expenses', $data)->row();
	}

	public function all()
	{
		$query = " SELECT *,
        (SELECT name FROM staff WHERE staff.staff_id = home_expenses.accountant_id) AS accountant
        FROM home_expenses WHERE deleted = false";
